...
#원형은 사람을 물체로 했을때 양사이드에 빈공간이 많아 히트박스 표시가없다면 왜 충돌했는지 알기 어려웠다
#그에 반해 AABB는 직사각형의 구조로 만들수있어 원형의 비해 물체와 히트박스 간의 빈공간이 적어 좀더 충돌이 자연스러웠다

#2. AABB vs OBB: 회전 시 어떤 차이가 있는가?
#AABB는 히트박스가 회전을 하지않아 물체가 회전시 충돌이 되어야하는 부분인데 안되는 문제가 발생했지만
#OBB는 물체가 회전시 히트박스도 동시에 회전하기에 여러가지 색다른 충돌이 가능했으며 자연스럽게 충돌하였다 

#3. 어떤 방식이 내 게임에 더 적합할까?
#회전하지 않는다면 AABB도 고려해볼수있고 회전까지 하게 된다면 OBB가 충돌에 대해서는 유저가 예상할수있는 좋은 경험을 준다

#4. AI에게 물어본 것 중 흥미로웠던 답변은?
#OBB가 모든 상황에서 좋지만 계산복잡도가 높아 회전하지 않는 경우에는 AABB 추천한것이 코딩에서는 한가지가 무조건 좋다라는 생각보다는 상황에 따라 좋은것이 다르다라는 것을 알게 되어 흥미로웠다
...
pygame.display.set_caption("OBB Smooth Rotation")
...
# 🔥 이미지 로드
# =========================
def load_image(base64_string, size):
...
player_img = load_image(_ADVENTURER, (60, 80))
enemy_img = load_image(_ROCKET, (80, 80))
...
# =========================
enemy_angle = 0
rotation_speed = 90 / 3000  # ms당 0.03도
